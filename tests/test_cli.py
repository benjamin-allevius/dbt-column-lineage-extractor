...
from src.dbt_column_lineage_extractor.cli_direct import main as direct_main
from src.dbt_column_lineage_extractor.cli_recursive import main as recursive_main
...
def test_cli_direct_basic(test_data_dir, test_output_dir):
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args:
        ...
        assert os.path.exists(
            os.path.join(test_output_dir, "lineage_to_direct_parents.json")
        )
        assert os.path.exists(
            os.path.join(test_output_dir, "lineage_to_direct_children.json")
        )
...
def test_cli_recursive(test_data_dir, test_output_dir):
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args:
        ...
        mock_args.return_value.lineage_parents_file = os.path.join(
            test_output_dir, "lineage_to_direct_parents.json"
        )
        mock_args.return_value.lineage_children_file = os.path.join(
            test_output_dir, "lineage_to_direct_children.json"
        )
        mock_args.return_value.output_dir = str(test_output_dir)
...
def test_cli_direct_with_model_list_json(test_data_dir, test_output_dir):
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args:
        ...
        mock_args.return_value.model = (
            []
        )  # Empty list as this should be overridden by model_list_json
