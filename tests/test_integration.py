...
def test_end_to_end_workflow(test_data_dir, temp_output_dir):
    ...
    descendants_file = os.path.join(
        temp_output_dir, f"{model}_{column}_descendants.json"
    )
    ...
    with patch("argparse.ArgumentParser.parse_args") as mock_args:
        ...
        with patch("argparse.ArgumentParser.parse_args") as mock_args_recursive:
            ...
            ancestors_file = os.path.join(
                temp_output_dir, f"{model}_{column}_ancestors.json"
            )
            descendants_file = os.path.join(
                temp_output_dir, f"{model}_{column}_descendants.json"
            )
...
def test_complex_model_relationships(test_data_dir, temp_output_dir):
    ...
    children_file = os.path.join(
        temp_output_dir, "full_lineage_to_direct_children.json"
    )
    ...
    if leaf_models:
        ...
        ancestors_file = os.path.join(
            temp_output_dir, f"{leaf_model}_{first_column}_ancestors.json"
        )
        with open(ancestors_file, "w") as f:
            json.dump(ancestors, f, indent=4)
    ...
    if root_models:
        ...
        if root_model in children_lineage:
            ...
            descendants_file = os.path.join(
                temp_output_dir, f"{root_model}_{first_column}_descendants.json"
            )
            with open(descendants_file, "w") as f:
                json.dump(descendants, f, indent=4)
